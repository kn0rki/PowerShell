...
$iplist = @()
if ($iZip -and !($update)) {
    Get-EsxImageprofile -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
} else {
    if ($v65) {
        Get-EsxImageprofile "ESXi-6.5*" -Softwaredepot $basedepot | foreaForEach-Objectch { $iplist += $_ }
    } else {
        if ($v60) {
            Get-EsxImageprofile "ESXi-6.0*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
        } else {
            if ($v55) {
                Get-EsxImageprofile "ESXi-5.5*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
            } else {
                if ($v51) {
                    Get-EsxImageprofile "ESXi-5.1*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
                } else {
                    if ($v50) {
                        Get-EsxImageprofile "ESXi-5.0*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
                    } else {
                        ...
                        Get-EsxImageprofile "ESXi-5.5*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
                        Get-EsxImageprofile "ESXi-6.0*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
                        Get-EsxImageprofile "ESXi-6.5*" -Softwaredepot $basedepot | ForEach-Object { $iplist += $_ }
                    }
                }
            }
        }
...
if ( ($pkgDir -ne "") -or $update -or ($load -ne @()) -or ($remove -ne @()) ) {
    ...
    if ($update) {
        ...
        $diff.UpgradeFromRef | ForEach-Object {
            $uguid = $_
            $uvib = Get-EsxSoftwarePackage | Where-Object { $_.Guid -eq $uguid }
    ...
    if ($pkgDir -ne "") {
        ...
        foreach ($obundle in Get-Item $pkgDir\*.zip) {
            ...
            if ($ob = Add-EsxSoftwaredepot $obundle -ErrorAction SilentlyContinue) {
                write-host -ForegroundColor Green " [OK]"
                $ob | Get-EsxSoftwarePackage | ForEach-Object {
