...
foreach ($computer in $computers) {
	...
	$TopProcesses = Get-Process -ComputerName $computer | Sort-Object WS -Descending | Select-Object ProcessName, Id, WS -First $ProccessNumToFetch | ConvertTo-Html -Fragment
	
	#region Services Report
	$ServicesReport = @()
	$Services = Get-WmiObject -Class Win32_Service -ComputerName $computer | Where-Object {($_.StartMode -eq 'Auto') -and ($_.State -eq 'Stopped')}
